...
class NcclLibEval(BaseEval):
    name:str
    type: Literal["vetnode.evaluations.nccl_on_rccl_lib_eval.NcclOnRcclLibEval"]
    requirements: Literal[[['hip-python-as-cuda~=7.0.2','--index-url','https://test.pypi.org/simple'],"numpy"]]
    ...
    def verify(self)->bool:
        libs =["/opt/rocm/lib/librccl.so"]   #add lib libnccl-net.so "libnvrtc.so"
        for lib in libs:
